...
cycle. Same incremental-fetch loop as the base driver (the model gathers tools itself --
NO giant pre-seeded packet, which is what pushed the KOFF runs past Together's serverless
worker limit and gave a ruminating model a huge surface to spin on).
...
from forecaster import awc, climo, store, tafgen, tools
        ...
        for tc in tcs:
            ...
            if name == "emit_taf" and res.taf is not None:
                ...
                if not tafgen.validate(res.taf):
                    final_taf = res.taf
            # Full data capture for the log: the complete tool receipt, and for the sinks
            # the full submitted arguments (the worksheet the model actually built).
